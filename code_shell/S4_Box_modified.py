...
NameCAE=ModelCase+'_BoxModel.cae'
NameModelin=ModelCase+'_BoxModel'
...
session.writeFieldReport(fileName=NameModel+'_S_local.out', append=OFF,
    sortItem='Element Label', odb=odbObj, step=0, 
    frame=vps.odbDisplay.fieldFrame[-1], outputPosition=INTEGRATION_POINT, 
...
session.writeFieldReport(fileName=NameModel+'_E_local.out', append=OFF,
    sortItem='Element Label', odb=odbObj, step=0,
    frame=vps.odbDisplay.fieldFrame[-1], outputPosition=INTEGRATION_POINT, 
    variable=(('LE', INTEGRATION_POINT, ((
    COMPONENT, 'LE11'), (COMPONENT, 'LE22'),(COMPONENT, 'LE12'), 
    (INVARIANT, 'Max. Principal'), )), ),
    stepFrame=SPECIFY)

## Rotate to global coordinate system for exporting field variables 
